...
import { sendEditingPermissionTransferred } from '@/lib/resend';
...
export async function POST(
  ...
  try {
    ...
    if (!hasPermission) {
      return NextResponse.json(
        { error: 'No tienes permisos para transferir la edición' },
        { status: 403 }
      );
    }
    ...
    if (transfer.from_user_id !== userId) {
      return NextResponse.json(
        { error: 'Solo el emisor puede transferir permisos' },
        { status: 403 }
      );
    }

    // Actualizar editing_permission_user_id al receptor
    await executeQuery(
      `UPDATE transfers 
      SET editing_permission_user_id = $1, updated_at = now()
      WHERE id = $2`,
      [transfer.to_user_id, transferId]
    );

    // Enviar email de notificación
    try {
      await sendEditingPermissionTransferred(
        transfer.to_email,
        transfer.hash,
    ...
    console.error('[transfers/transfer-permission] Error:', error);
    return NextResponse.json(
